...
WHEN NEW.status IN ('releasable', 'published')
 ...
 AND NOT EXISTS (
     SELECT 1
     FROM reading_review_receipt AS receipt
     JOIN reading_review_authority_input AS authority
       ON authority.certificate_urn = receipt.certificate_urn
      AND authority.certificate_hash = receipt.certificate_hash
      AND authority.run_id = receipt.run_id
      AND authority.run_artifact_hash = receipt.run_artifact_hash
      AND authority.requirements_manifest_hash = receipt.requirements_manifest_hash
      AND authority.review_artifact_hash = receipt.review_artifact_hash
      AND authority.review_set_hash = receipt.review_set_hash
      AND authority.reviewer_identity_hash = receipt.reviewer_identity_hash
     JOIN paper_lab_event AS event
...
BEGIN
    SELECT RAISE(ABORT, 'reading release requires an independent review authority input');
END;
