...
WRITER_HANDOFF_JOURNAL_SCHEMA = "qrh-writer-handoff-pending/v2"
...
def _expected_candidate_paths(root: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "publish_recovery_cli_module": service_host.with_name(
            "publish_recovery_cli.py"
        ),
...
def prepare_service_host_environment(root: Path) -> Path:
    """Confine SCM-host temp/cache writes before it resolves active content."""

    physical = root.resolve(strict=True)
    if PureWindowsPath(str(physical)) != PRODUCTION_VM_ROOT:
        raise WindowsServiceError(r"service host root must be D:\quant\quant_platform")
    tmp = physical / "tmp" / "service-host"
    pycache = tmp / "pycache"
    for path in (tmp, pycache):
...
def _append_service_host_status(root: Path, status: str) -> None:
    """Keep minimal host lifecycle evidence in D-root, never Event Log."""

    if not status or any(character not in "abcdefghijklmnopqrstuvwxyz_0123456789" for character in status):
        status = "host_failure"
    path = root / "logs" / "quant-research-hub-service.log"
    validate_production_vm_write_path(str(path), allow_root=False)
...
def resolve_active_service_release(root: Path) -> ActiveServiceRelease:
    ...
    DeploymentController(physical).verify_finalized_release(
...
def authorize_writer_handoff_service_start(
    ...
    if (
        ...
        or (
            value.get("phase") == "d_start_authorized"
            and value.get("commit_evidence") is not None
        )
        or (
...
def parse_service_start_authorization(args: object) -> ServiceStartAuthorization | None:
    ...
    if (
        ...
        or tail[1] not in {"candidate", "prior_recovery"}
...
class ServiceSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        activation_authorization: ServiceStartAuthorization | None = None,
    ) -> None:
...
class ServiceInstallCandidate:
    ...
    publish_recovery_cli_module: Path
    publish_recovery_cli_module_sha256: str
    ...
    def document(self) -> Mapping[str, object]:
        return {
            ...
            "publish_recovery_cli_module": str(self.publish_recovery_cli_module),
            "publish_recovery_cli_module_sha256": self.publish_recovery_cli_module_sha256,
...
def build_install_candidate(root: Path, service_name: str) -> ServiceInstallCandidate:
    ...
    publish_recovery_cli_module = expected["publish_recovery_cli_module"]
    ...
    ensure_no_reparse_components(publish_recovery_cli_module)
    ...
    if (
        ...
        or not publish_recovery_cli_module.is_file()
    ...
    if PureWindowsPath(str(root.resolve())) == PRODUCTION_VM_ROOT:
        ...
        validate_production_vm_write_path(str(publish_recovery_cli_module), allow_root=False)
    ...
    return ServiceInstallCandidate(
        ...
        publish_recovery_cli_module=publish_recovery_cli_module,
        publish_recovery_cli_module_sha256=_hash_file(publish_recovery_cli_module),
...
def apply_install_candidate(
    ...
    if (
        ...
        or _hash_file(candidate.publish_recovery_cli_module)
        != candidate.publish_recovery_cli_module_sha256
...
try:  # Windows-only service host; imports stay optional for non-Windows CI.
    ...
    class QuantResearchHubWindowsService(win32serviceutil.ServiceFramework):
        ...
        def __init__(self, args):
            ...
            prepare_service_host_environment(root)
            super().__init__(args)
        ...
        def SvcDoRun(self):
            try:
                ...
                return_code = self.supervisor.wait()
            except Exception as error:
                _append_service_host_status(
            ...
            if return_code:
                _append_service_host_status(
                    self.supervisor.root, f"child_exit_{return_code}"
                )
