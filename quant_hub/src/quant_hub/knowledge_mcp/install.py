...
- 单个任务只做一个聚焦 search；最多 get 其 `next_action` 返回的 1–3 个、影响决定的唯一 ID。不要猜 ID、重复 search、重复展开同一对象或为“覆盖全面”读取全部结果。
...
- 最终研究建议必须明确区分“证据支持的决定、适用条件、限制/失败经验、source identity/引用”，并逐项使用 get 返回的 canonical `source_citations`；证据没有覆盖的部分要显式说明不足，不得用模型常识补齐成项目历史结论或编造 citation ID。
