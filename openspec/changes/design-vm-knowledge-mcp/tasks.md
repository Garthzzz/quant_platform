...
- [ ] 5.14 用相同任务做 MCP-assisted 与 no-MCP 对照；只有 grounded decision、条件/限制识别和引用正确性出现可复现净增益且无意义调用受控时才通过。
  - 后续只能使用运行前生成的封闭 `qrh-mcp-acceptance-preregistration/v1`：绑定 prompt bytes hash/length、tool-choice 顺序与预算，以及三项质量 marker 定义的 canonical bytes/hash；每维只接受 nonempty unique `list[str]`/`tuple[str]`，拒绝 string/bytes 标量及其自洽伪 envelope。历史 v2/v3 不具备该合同，保持不可重算 FAIL；禁止回填 marker/prereg 或据此补造 PASS。
