...
- [x] 2.4a 将未就绪的 failure-domain authority 收缩为全产品只读诊断边界：已安装产品包移除 current/archive/completion writer、CAS、锁、原子替换与恢复 core；唯一 v2 authority API 在文件访问前固定 `FAILURE_DOMAIN_AUTHORITY_NOT_READY`、`authority=false`。publish、cold bundle、publish recovery、cold restore/qualification reset、Stage 5、state-only、Scheduler 与 writer handoff 全部迁移到该入口，禁止由 fresh/self-consistent legacy v1 或旧 receipt 生成正式 authority。legacy synthetic lineage、facts/probe/attestation 与 source manifest 仅为 `DIAGNOSTIC_ONLY`；AST、源码真实入口与 fresh installed-wheel 调用图测试证明无产品 fallback、旧 CLI 零写入、正式 consumer 不直接调用 `attest_failure_domain`。Round6 进一步覆盖安装 wheel 的 `qrh-publish` dry/non-dry、`qrh-writer-handoff-client` run/status/finalize 及 publish-recovery 全部分支：均复用同一 exception document 输出 closed `NOT_READY` JSON/exit 2，且 open/read/config/Git/remote/VM snapshot spies 为零；五个 recovery public API 自身也以 gate 为首条有效语句。Round7 建立 formal release/recovery/handoff 导出 surface 的 closed 分类：队列/pipeline/runtime/recovery adapter/deploy/writer client/Windows OS adapter 等 side-effect 或 authority callable 首句 gate，构造不提前创建目录；本地 Git/dry-run/existing-directory inventory 只读诊断和 exact-SHA CI/test/public guard 资格输入不误封，且不产 authority。Round8 取消 `__all__` 对清单的过滤，source/fresh wheel 精确枚举非私有定义、模块 callable alias、class 公开 descriptor/method/`__call__` 与内部正式 factory；配置、协议、固定拒绝 support 和 exact Git push 均唯一分类，计数由 inventory 机械生成，三个实际边界 runner 私有化且旧名不可导入，`ExactGitPush.__call__` 在 process 前固定 gate。fresh wheel 对全部 surface 做 AST exact inventory，对高风险共享路径做 config/path/tree/subprocess/HTTP/remote/OS 零调用 spies，诊断树保持 byte identity；新增公开 surface 未分类即失败。changed-file manifest 使用 UTF-8/NFC repo-relative path 排序及 `path NUL sha256hex NUL` 可独立复算。未来 integrated runner 必须在独立 module 中以新 schema、不可序列化 capability、exact Git/CI/wheel 和 SSH host authentication 重新设计；本项不代表真实 VM 刷新、qualification 或 2.4 已放行。
...
- [ ] 5.13 交付 cwd 无关、可幂等 install/doctor/uninstall 的 CLI/package、user/project Codex profile、`AGENTS.md`/server instructions 应调用与不应调用规则；在 `quant_platform` 和独立 `D:\quant\backtest_demo` 从无显式 MCP 字样的 prompt 验证 search→get、不应调用、snapshot activation/rollback 后 list-updates→重查与返回身份。本版不实现 HTTP。
  - 2026-08-22 冻结验收：install/doctor/uninstall、cwd 无关、隐式初始 search→get、无关任务零调用、fresh/stale/unavailable 和 exact identity 已有真实 CLI 证据；但初始出现一次非 search 返回 ID 的 get，v3 activation/rollback 分别为 35/20 次目标调用且各有一次非法 `budget_chars` get，超过预注册上限 6，故本任务保持未完成。去敏机器报告位于 Git 外 `D:\quant\quant_platform_recovery\mcp-acceptance\20260822-frozen-v111\evidence\acceptance-report.json`。
