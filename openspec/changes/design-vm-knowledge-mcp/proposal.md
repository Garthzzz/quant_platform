...
- 将生产 VM 的闭合写入根固定为 `D:\quant\quant_platform`：checkout、candidate、release、state、backup、audit、lock、log、tooling、TEMP/TMP 和 bytecode 均不得写到 `D:\`、`D:\quant`、sibling/parent 或 C 盘；执行前后均须机械验证并审计。
...
- 自动生成无 secret、机器可验证的 cold recovery bundle；除 release/content/resource/index 与 SQLite checkpoint 外，还密封空 D 启动所需的 `tooling/python`、固定控制面和 service install candidate operational closure，凭据在恢复后由受保护渠道重注入。最终 `RECOVERY_ROOT` 必须实测位于生产 VM 整机之外，同一 VM 其他盘符不合格；恢复执行只使用唯一目标 `.240`，不依赖第二恢复 VM。V39 从真实空 D 恢复通过前禁止首次生产切换和旧恢复材料清理；最终 release 再演练一次。唯一 state-only job 至少每 24 小时生成新的 immutable checkpoint/recovery receipt，RPO 以最后成功验证 checkpoint 的实际年龄判断，超龄进入 degraded/failed 并告警。激活前只生成 `recovery_protection_receipt`；pointer 前的 durable pending journal 只用于 crash coordination，不是第二 authority，服务启动需精确 SCM transient authorization；成功切换并完成验证后才生成 `activation_receipt`，切换失败只生成 failure receipt。
...
- 本 change 已获用户批准进入 apply；实施仍必须遵守空 D recovery、writer handoff、Stage 5 certificate 与 Public→Private 的既定硬门禁。
...
影响 `quant_hub` 的 archive/integration/collaboration/web/search 边界及新增 knowledge/MCP/ops 边界、VM 唯一可写根 `D:\quant\quant_platform`、GitHub CI，以及客户端本地 stdio、可跨项目安装的 Codex profile。`reference/**` 与 `D:\quant\industry_demo` 保持只读；现有前端不重做；旧 C 盘线上服务在新 D 盘候选获批前保持不变且只读核验。
