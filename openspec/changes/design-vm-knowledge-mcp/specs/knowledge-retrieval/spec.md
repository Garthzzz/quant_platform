...
系统 SHALL 组合 exact ID/alias/title、FTS5、CJK n-gram/trigram、短词 fallback、applicability、关系扩展、版本/状态惩罚和确定性重排，并返回命中原因、限制、反例及 source locator。
...
检索 SHALL 使用由确定性 IR 产生的 heading-aware chunks，并携带 research/document/source version、heading path、block type、byte/line locator、citation IDs、source/chunker hash、active/deprecated 状态及可用 applicability/fact/relation metadata；公式、表格、代码和引用不得在语义边界中间切断。去重 SHALL 以 canonical source span、知识簇和 citation chain 为准，而不是让 overlap 或多路命中提高同一证据权重。

#### Scenario: 同一 span 同时被 lexical、结构化知识和关系扩展命中
- **WHEN** 三路候选都指向相同 canonical evidence
- **THEN** 系统 SHALL 合并为一张 evidence card，保留各命中原因但只计一次证据权重
...
- **THEN** 每张卡片 SHALL 来自同一 snapshot 的 accepted knowledge，显示 fact status 与 source locator；当前 generation pending/blocked/failed 时页面 SHALL 显示状态而不得从旧 version 或未验证 candidate 填充
...
系统 SHALL 使用覆盖因子、模型、数据处理和回测的 development set 与至少三分之一 sealed holdout；每条 qrel SHALL 绑定 task context、answerability、expected/forbidden method、适用/冲突 facets、positive/negative source spans、当前 source version 与必需引用，来源修订后 SHALL 自动 stale 并等待重新裁决。调参不得查看 holdout，评测 SHALL 含 hard negative、无答案、条件冲突、历史/废弃、错引、跨语言和公式别名切片。
